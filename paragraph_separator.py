...
class ParagraphSeparator:
    ...
    # 一行かどうか
    def is_one_sentence(self, paragraph, spans):
        return len(spans) == 0 or spans[0][1] == len(paragraph)

    # 最大行数以内か
    def is_less_than_max_line_num(self, paragraph, spans):
        return self.calc_line_num(paragraph, spans) <= self.max_line_num

    # 段落をできるだけ均等になるように二分割する
    def separate_to_two_paragraph(self, paragraph, spans):
    ...
    # 段落の表示に必要な行数を算出
    def calc_line_num(self, paragraph, spans):
        line_num = 0
        prev_pos = 0
        for span in spans:
